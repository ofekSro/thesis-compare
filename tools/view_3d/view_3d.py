...
Usage:
    python tools\\view_3d\\view_3d.py                      # tkinter picker
    python tools\\view_3d\\view_3d.py config_01_det1_b15_s5_h4_w50

A tkinter dialog lets you choose a configuration (skipped if one is given on
the command line), then pyvista shows the 3D scene with a checkbox to toggle
between Peak Overpressure and Peak Impulse.

Requires the optional dependency pyvista (see requirements-optional.txt).
"""
...
def select_configuration(npz_dir, obs_npz_dir):
    """Show a tkinter dialog to pick a config. Returns config name or None."""
    npz_dir = str(npz_dir)
    obs_npz_dir = str(obs_npz_dir)

    if not os.path.isdir(npz_dir):
        messagebox.showerror('Error', f'Folder not found: {npz_dir}')
        return None
    ...
    if not all_configs:
        messagebox.showerror('Error', f'No .npz files found in {npz_dir}')
        return None

    obs_configs = set(
        f.replace('.npz', '')
        for f in os.listdir(obs_npz_dir)
        if f.startswith('config_') and f.endswith('.npz')
    ) if os.path.isdir(obs_npz_dir) else set()
    ...
    status_var = tk.StringVar(
        value=f'{len(all_configs)} configs, {len(obs_configs)} with OBS data')
...
def build_obs_meshes(npz):
    ...
    Same logic as the grid processing: coarser grids are cut where finer grids
    ...
    for res in ('1', '2', '3'):
        ...
        res_data[res] = {
            ...
            'peakP': np.ascontiguousarray(npz.get(f'peakP_{res}', np.array([])),
                                          dtype=np.float32),
            'impulse': np.ascontiguousarray(npz.get(f'impulse_{res}', np.array([])),
                                            dtype=np.float32),
        }
...
def show_3d(config_name, ground_npz, obs_npz=None):
    ...
    # Exaggerate height so buildings are visible
    # (ground is ~100m wide, buildings ~4m tall)
    HEIGHT_SCALE = 5.0
...
def main(config_name=None, *, npz_dir=None, obs_npz_dir=None, progress=print):
    ...
    if config_name is None:
        config_name = select_configuration(npz_dir, obs_npz_dir)
        if config_name is None:
            progress('No configuration selected.')
            return None

    progress(f'Loading {config_name}...')

    ground_path = npz_dir / f'{config_name}.npz'
    if not ground_path.exists():
        progress(f'Error: {ground_path} not found.')
        return None
    ground_npz = np.load(ground_path, allow_pickle=True)

    obs_path = obs_npz_dir / f'{config_name}.npz'
    obs_npz = None
    if obs_path.exists():
        obs_npz = np.load(obs_path)
        progress('  OBS data loaded.')
    else:
        progress('  No OBS data for this config.')
...
def cli(argv=None):
    p = argparse.ArgumentParser(description='Interactive 3D field viewer.')
    p.add_argument('config_name', nargs='?', default=None,
                   help='Config to view (omit to open the picker dialog).')
    p.add_argument('--npz-dir', default=None, help='Folder of config_*.npz files.')
    p.add_argument('--obs-npz-dir', default=None, help='Folder of OBS .npz files.')
    args = p.parse_args(argv)
    return main(args.config_name, npz_dir=args.npz_dir, obs_npz_dir=args.obs_npz_dir)
