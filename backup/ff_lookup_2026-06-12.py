...
at scaled distances Z=1-20 for each charge weight) and to find the 95th-percentile
radius where urban field values match free-field values within a given tolerance,
using angular binning (91 bins covering 0-90 degrees).
"""
...
# Matching tolerance for free-field comparison (5%)
FF_TOLERANCE = 0.05
...
def _find_percentile_radius(all_vals, all_X, all_Zc, ff_val, tolerance, p=95):
    """Find the 95th-percentile radius for cells matching a free-field value.

    Identifies cells whose values are within ``tolerance * ff_val`` of the
    free-field reference value, bins them into 91 angular bins (0-90 degrees,
    1-degree spacing), finds the maximum distance in each bin, then returns
    the p-th percentile of those per-bin maxima.
    ...
    ff_val : float
        Free-field reference value to match against.
    tolerance : float
        Fractional tolerance for matching (e.g. 0.05 for 5%).
    p : float
        Percentile to compute (default 95).
    ...
    """
    match = ~np.isnan(all_vals) & (np.abs(all_vals - ff_val) <= tolerance * ff_val)
    if not np.any(match):
