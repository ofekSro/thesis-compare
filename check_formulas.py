...
from regression import _r2_mape
...
def load_nonlinear_coefficients(csv_path):
    ...
    valid for Z_free >= zf_min (1 pressure, 2 impulse).
    """
    df = pd.read_csv(csv_path)
    coeffs = {}
    for _, row in df.iterrows():
...
def main():
    ...
    # ---- Print best formulas ----
    _print_formulas(conv_coeffs, z_coeffs)
    ...
    for cfg_name in maxR_df['Config'].unique():
        ...
        if cfg:
            ...
            geo_cache[cfg_name] = {
                ...
                'height': cfg['height'], 'swidth': cfg['swidth'], 'rho': rho,
            }
    ...
    for _, row in maxR_df.iterrows():
        ...
        conv_pred_P, conv_pred_I = conv_pred_cache.get(cfg_name, (np.nan, np.nan))
        R_ff = z_val * W_third
        if np.isfinite(pred_mR_P):
            if np.isfinite(conv_pred_P):
                pred_mR_P = min(pred_mR_P, conv_pred_P)
            pred_mR_P = max(pred_mR_P, R_ff)
        if np.isfinite(pred_mR_I):
            if np.isfinite(conv_pred_I):
                pred_mR_I = min(pred_mR_I, conv_pred_I)
    ...
    print('  (derived canonically: R_urban = W^(1/3) * Z_urban)')
    print('=' * 60)
...
def _print_formulas(conv_coeffs, z_coeffs):
    ...
    for target in ['RadiusP', 'RadiusI']:
        ...
        for (det, tgt), c in sorted((k, v) for k, v in conv_coeffs.items() if k[1] == target):
            ...
            print(f'  {loc}:')
            if c['formula'] == 'log':
                print(f'    Z = {c["A"]:.4f} * Pi^({c["k"]:+.4f}*ln(W^1/3/s))')
            elif c['has_H_term']:
    ...
    for target in ['Pressure', 'Impulse']:
        for det in [1, 2]:
            ...
            print(f'    Z_urban = {c["C"]:.4f} * Z_free^{c["m"]:.4f}'
                  f' * rho^{c["p"]:+.4f} * (H/s)^{c["q"]:+.4f}'
                  f' * (s/W^1/3)^{c["r"]:+.4f}')
            print(f'    MaxR = W^(1/3) * Z_urban    [Z_free >= {c["zf_min"]:g}]')
